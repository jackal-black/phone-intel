...
| 能力 | 对应实现 |
|------|----------|
| **Workflow 设计** | Prefect `@flow`/`@task` DAG 编排、任务依赖拓扑 |
| **异步编程** | `asyncio` + `httpx.AsyncClient` 非阻塞 IO |
| **容错工程** | retry/backoff/timeout/circuit-breaker/fallback |
| **LLM 应用** | Prompt 工程、结构化输出、空结果跳过、降级 |
| **API 集成** | Tavily Search、DeepSeek Chat、Feishu Webhook |
| **工程规范** | 类型注解、.env 配置、模块化、日志、try/except |

### 面试应答

> **Q: 这个项目的 workflow 体现在哪里？**
> 
> A: "我用 Prefect 把整个流程拆成 4 个 Task，**定义了清晰的 DAG 依赖拓扑**——4 个搜索 Task 并行执行，全部完成后触发分析 Task，分析完再推送。每个 Task 有独立的**重试策略、超时阈值、重试间隔**，并由 Prefect Server 统一追踪。这不是一个脚本，而是一个**可观测、可恢复、可独立重跑**的工作流。"

> **Q: 为什么 async 比 sync 适合这里？**
> 
> A: "本项目是 IO 密集型——4 个搜索请求是天然并行的。sync 版本串行跑要 8 秒，async 并行 3 秒搞定。而且 async task 的 timeout 是**可中断的**，sync task 在 Prefect 里 timeout 只是个摆设。所以 async 不是炫技，是实实在在的 IO 优化和可靠性提升。"

### 关键词

```
Workflow Engine · DAG · Task Orchestration · asyncio · httpx
Prefect · Retry Policy · Timeout Control · Conditional Branching
Cron Scheduling · Observability · LLM Pipeline · Prompt Engineering
Tavily API · Feishu Webhook · Python · Type Hints
```
